...
export default class FuncionarioRepository {
  ...
  create(funcionario, callback) {
    ...
    return this.connection.query(sql, [funcionario.id, funcionario.nome, funcionario.cpf, funcionario.telefone, funcionario.email, funcionario.Cargo_id], callback);
  }

  update(id, funcionario, callback) {
    const sql = 'UPDATE Funcionario SET nome = ?, cpf = ?, telefone = ?, email = ?, Cargo_id = ? WHERE id = ?';
    return this.connection.query(sql, [funcionario.nome, funcionario.cpf, funcionario.telefone, funcionario.email, funcionario.Cargo_id, id], callback);
  }
